...
- **A burned release stays burned.** release-please emits `paths_released` once per release, so a publish that fails after the tags are cut strands that version; the recovery is to cut the next one. This is deliberate, not a gap to engineer around: the version a release carries is derived from its commits, and a hand-triggered republish path would be a second way to publish that no commit accounts for.
