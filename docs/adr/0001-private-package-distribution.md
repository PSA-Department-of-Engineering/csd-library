...
- **npm consumers depend on the scoped name.** Their code can import the scoped name directly, or use an `npm:` alias (`"vitest-intent": "npm:@psa-department-of-engineering/vitest-intent@^0.2.0"`) to keep import specifiers namespace-neutral. The scope reference is then confined to `package.json` + `.npmrc`.
...
- **Migration.** Consumers move off `file:` / `-e` to versioned deps (npm `^X.Y.Z`, pip git-tag URL). `npm link` / `pip install -e` remain available as a local-dev escape hatch for working against unreleased changes, but the committed state is always the versioned dependency.
